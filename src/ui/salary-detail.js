...
export function salaryDetailMarkup(decision, { fmtMoney, currentSalary = 0, isProfessional = false, contract = null } = {}) {
  const offer=contract?.offerBreakdown;
  const contractMarkup=contract?.schemaVersion===3?`<h3>現契約</h3><dl><dt>契約種別</dt><dd>${escapeHtml(contract.contractType)}</dd><dt>契約期間</dt><dd>${contract.startYear}～${contract.endYear}年</dd><dt>保障総額</dt><dd>${escapeHtml(fmtMoney(contract.guaranteedTotal))}</dd><dt>出来高</dt><dd>${contract.incentive?`最大${escapeHtml(fmtMoney(contract.incentive.annualMax))}／年`:'なし'}</dd><dt>支払済</dt><dd>${escapeHtml(fmtMoney(contract.paidTotal))}</dd><dt>残り保障</dt><dd>${escapeHtml(fmtMoney(contract.remainingValue))}</dd></dl>${offer?`<h3>FAオファー内訳</h3><dl><dt>市場基準額</dt><dd>${escapeHtml(fmtMoney(offer.marketSalary))}</dd><dt>故障補正</dt><dd>×${Number(offer.injuryMultiplier).toFixed(2)}</dd><dt>守備位置係数</dt><dd>×${Number(offer.positionMultiplier).toFixed(2)}</dd><dt>契約タイプ係数</dt><dd>×${Number(offer.contractTypeMultiplier).toFixed(2)}</dd><dt>球団需要</dt><dd>×${Number(offer.teamDemandMultiplier).toFixed(3)}</dd><dt>競合補正</dt><dd>×${Number(offer.competitionMultiplier).toFixed(3)}</dd><dt>最終年俸</dt><dd>${escapeHtml(fmtMoney(offer.finalAnnualSalary))}</dd></dl>`:''}${contract.contractType==='PROOF'?'<p>再起を懸ける1年契約（証明契約）<br>1年／固定年俸／満了後に再評価</p>':''}<div class="contract-schedule">${contract.annualSchedule.map(item=>`<div><span>${item.year}年</span><span>${escapeHtml(fmtMoney(item.amount))}</span><small>${item.paid?'支払済':'予定'}</small></div>`).join('')}</div><h3>契約セグメント</h3><div class="contract-segments">${contract.segments.map(segment=>`<div>${escapeHtml(segment.type)}　${segment.startYear}～${segment.endYear}年　${escapeHtml(fmtMoney(segment.annualSalary))}／年</div>`).join('')}</div>`:'';
  ...
  const stageLabel=decision.contractStage==='ARBITRATION'?'MLB年俸調停対象':decision.contractStage==='CONTROL'?`${decision.org||''}球団保有期間`:'FA契約対象';
  ...
  return `<div class="salary-detail-summary"><strong>${decision.salaryYear}年 年俸　${escapeHtml(fmtMoney(decision.finalSalary))}</strong><br>前年　　　　 ${escapeHtml(fmtMoney(decision.previousSalary))}<br>増減　　　　 ${escapeHtml((decision.changeAmount >= 0 ? '+' : '−') + fmtMoney(Math.abs(decision.changeAmount)))}${decision.changeRate === null ? '（新規契約）' : `（${signed(decision.changeRate * 100, 1)}%）`}</div>`+
    ...
    `<h3>契約・守備</h3><dl><dt>契約段階</dt><dd>${escapeHtml(stageLabel)}（在籍${Number(decision.serviceYears)||0}年）</dd><dt>健康状態</dt><dd>${escapeHtml(({HEALTHY:'健康',MINOR:'軽い故障',MAJOR:'大きな故障',REHAB:'リハビリ'})[decision.marketInjury]||'健康')}</dd><dt>市場評価</dt><dd>${Number(decision.sourceMarketRating||0).toFixed(2)}</dd><dt>故障補正</dt><dd>×${Number(decision.injuryMultiplier||1).toFixed(2)}</dd><dt>減俸保護</dt><dd>${decision.decreaseProtectionApplied?'前年の75%':'適用なし'}</dd><dt>基準年俸</dt><dd>${escapeHtml(fmtMoney(decision.baseSalary))}</dd><dt>守位係数</dt><dd>×${Number(decision.positionMultiplier).toFixed(2)}</dd><dt>契約係数</dt><dd>×${Number(decision.contractMultiplier).toFixed(2)}</dd><dt>最終年俸</dt><dd>${escapeHtml(fmtMoney(decision.finalSalary))}</dd></dl>${decision.arbitration?`<h3>年俸調停結果</h3><dl><dt>球団提示</dt><dd>${escapeHtml(fmtMoney(decision.arbitration.clubSalary))}</dd><dt>選手側</dt><dd>${escapeHtml(fmtMoney(decision.arbitration.playerSalary))}</dd><dt>勝訴確率</dt><dd>${Number(decision.arbitration.winChance).toFixed(0)}%</dd><dt>結果</dt><dd>${escapeHtml(decision.arbitration.result)}</dd></dl>`:''}`+
