...
export function salaryDetailMarkup(decision, { fmtMoney, currentSalary = 0, isProfessional = false, contract = null } = {}) {
  const contractMarkup=contract?.schemaVersion===2?`<h3>現契約</h3><dl><dt>契約種別</dt><dd>${escapeHtml(contract.contractType)}</dd><dt>契約期間</dt><dd>${contract.startYear}～${contract.endYear}年</dd><dt>保障総額</dt><dd>${escapeHtml(fmtMoney(contract.guaranteedTotal))}</dd><dt>支払済</dt><dd>${escapeHtml(fmtMoney(contract.paidTotal))}</dd><dt>残り保障</dt><dd>${escapeHtml(fmtMoney(contract.remainingValue))}</dd></dl>${contract.contractType==='PROOF'?'<p>再起を懸ける1年契約（証明契約）<br>1年／固定年俸／満了後に再評価</p>':''}<div class="contract-schedule">${contract.annualSchedule.map(item=>`<div><span>${item.year}年</span><span>${escapeHtml(fmtMoney(item.amount))}</span><small>${item.paid?'支払済':'予定'}</small></div>`).join('')}</div><h3>契約セグメント</h3><div class="contract-segments">${contract.segments.map(segment=>`<div>${escapeHtml(segment.type)}　${segment.startYear}～${segment.endYear}年　${escapeHtml(fmtMoney(segment.annualSalary))}／年</div>`).join('')}</div>`:'';
