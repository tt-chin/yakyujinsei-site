...
import { runChoiceAction } from '../ui/choice-action.js';
...
function choose(title,opts){
  ...
  opts.forEach(o=>{ const b=document.createElement('button');
    ...
    b.onclick=()=>runChoiceAction({action:o.f,currentMarkup:()=>a.innerHTML,clear:actClear,restore:()=>choose(title,opts),reportError:()=>{card('bad','処理中にエラーが発生しました','選択処理を完了できませんでした。原因はコンソールに記録されています。もう一度選択するか、最初からやり直してください。');actToggleSync();}}); a.appendChild(b); });
