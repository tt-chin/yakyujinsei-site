...
(() => {
  ...
  function showFaMarket(market){
    if(!market.offers.length){const rating=market.marketRating,injury=injurySalaryMultiplier(S.marketInjury,isRecentStar(S.salaryEvaluationHistory||[])),annual=roundToTenThousandYen(Math.max(salaryFor(S.lv,rating)*dpMult()*.9*injury,salaryFor(S.lv,0))),incentive=createIncentiveTerms({org:S.org,annualSalary});choose('FA市場・獲得オファーなし',[{t:`元球団と1年契約（年俸${fmtMoney(annual)}）`,main:true,s:'市場価×0.90。減俸保護は適用されない',f:()=>{const previousSalary=S.currentSalary,base=salaryCandidate(),candidate={...base,contractMult:.9*injury,annualSalary};S.ct=fixedContract({org:S.org,teamId:S.orgTeamId,years:1,annualSalary,contractType:'FA_RETURN',candidate,incentive});S.currentSalary=annual;market.acceptedOfferId='RETURN_NO_BID';saveSalaryDecision('RENEWAL',candidate,previousSalary,annual);advance();}},{t:'現役を退く',warn:true,f:()=>endGame('FA市場で獲得オファーがなく、現役を引退した。')}]);return;}
