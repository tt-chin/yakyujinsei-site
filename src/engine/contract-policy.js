...
export function createContract({contractId,org,teamId,signedYear,startYear,years,annualSalary,contractType='NORMAL',signedMarketRating=0,positionMultiplierAtSigning=1,contractMultiplier=1,injuryProtection='FULL'}) {
  ...
  return deriveContractTotals({schemaVersion:2,contractId,org,teamId,signedYear:Number(signedYear),startYear:Number(startYear),endYear:Number(startYear)+count-1,years:count,remainingYears:count,contractType,annualSalary:annual,annualSchedule,segments,guaranteedTotal:annual*count,paidTotal:0,signedMarketRating:Number(signedMarketRating)||0,positionMultiplierAtSigning:Number(positionMultiplierAtSigning)||1,contractMultiplier:Number(contractMultiplier)||1,injuryProtection,extOffered:false});
}

export function normalizeContract(contract,fallback={}) {
  if(!contract)return null;
  if(contract.schemaVersion===2)return deriveContractTotals({...contract,segments:(contract.segments||[]).map(x=>({...x}))});
